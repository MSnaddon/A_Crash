...
const areas = {
  darkRoom: require("./areas/darkRoom"),
  cryoLab: require("./areas/cryoLab")
}

const startEvents = [
  new Event("You are cold, and the room is dark."), 
  new Event("Feeling around, your hands fall upon a Bio-light")
  ]

const startInventory = {
  test: {
    itemOne: new InventoryItem("Item 1", 5)
  }
}

const progressions = {
  litBioorb: new Progression("Bioorb is lit", 
    new Action("#remove dark room, show lit room", function(){
      this.areas.cryoLab.available = true
      this.areas.darkRoom.available = false
      this.eventFeed.push(new Event("Light pours into the room"))
    },{
      orbIsLit: function(){
        return this.areas.darkRoom.actions.turnBioLightOn.counter > 0;
      }
    })
  )
}
...
gameOptions.areas = areas;
gameOptions.eventFeed = startEvents;
gameOptions.progressions = progressions;
